...
import { API_MODULES, PRO_FEATURES, PRO_TOOLS, THEMES, canUseFeature, entitlementFor, isOwner, renderToolCatalog, safeApiPolicy, themeFor, tierPolicy, toolsByCategory } from './pro-tools.js';
...
const OWNER_IDS = String(process.env.BOT_OWNER_IDS || process.env.BOT_OWNER_ID || '').split(',').map((value) => Number(value.trim())).filter(Boolean);
const OWNER_ID = OWNER_IDS[0] || 0;
...
const proMenu = () => new InlineKeyboard().text('◈ Pro status', 'pro:status').text('▣ Tool catalog', 'pro:tools').row().text('◉ HUD themes', 'pro:themes').text('⌁ API modules', 'pro:apis').row().text('⌁ Pro features', 'tier:features').text('🧪 Tier test', 'tier:test').row().text('◀ Dashboard', 'menu:dashboard');
...
function toolPageKeyboard(page = 0) { const totalPages = Math.max(1, Math.ceil(PRO_TOOLS.length / toolPageSize)); const current = Math.min(Math.max(Number(page) || 0, 0), totalPages - 1); const keyboard = new InlineKeyboard(); PRO_TOOLS.slice(current * toolPageSize, (current + 1) * toolPageSize).forEach((tool, offset) => { keyboard.text(`${String(current * toolPageSize + offset + 1).padStart(2, '0')} · ${tool.name}`, `tool:view:${current * toolPageSize + offset}`).row(); }); if (current > 0) keyboard.text('◀ Previous', `tools:page:${current - 1}`); if (current < totalPages - 1) keyboard.text('Next ▶', `tools:page:${current + 1}`); keyboard.row().text('▦ Families', 'pro:tools').text('◀ Pro panel', 'pro:status'); return keyboard; }

async function isAdminFor(ctx, chatId, userId = ctx.from?.id) {
  if (OWNER_IDS.includes(Number(userId))) return true;
...
bot.command('pro', async (ctx) => { const tier = tierFor(ctx); const policy = tierPolicy(tier.tier); await ctx.reply(hud(ctx.chat?.id, 'PRO PLATFORM', `Tier: <b>${tier.label}</b>\nTools: <b>${PRO_TOOLS.length}</b> registered\nAPI modules: <b>${API_MODULES.length}</b> available\nTheme limit: <b>${policy.limits.customThemes === 1 ? 'Core theme' : policy.limits.customThemes === 5 ? 'All themes' : 'Unlimited'}</b>\n\n${tier.tier === 'CORE' ? '<i>Premium features are limited until this chat is granted Pro access.</i>' : '<i>Premium control surfaces are unlocked for this chat.</i>'}`), { parse_mode: 'HTML', reply_markup: proMenu() }); });
bot.command('profeatures', async (ctx) => { const tier = tierFor(ctx); await ctx.reply(hud(ctx.chat?.id, 'PRO FEATURES', PRO_FEATURES.map(([name, description]) => `${tier.tier === 'CORE' ? '🔒' : '◆'} <b>${name}</b> — ${description}`).join('\n')), { parse_mode: 'HTML', reply_markup: proMenu() }); });
bot.command('tiertest', async (ctx) => { const tier = tierFor(ctx); const policy = tierPolicy(tier.tier); await ctx.reply(hud(ctx.chat?.id, 'TIER TEST LAB', `Detected tier: <b>${tier.label}</b>\nAPI access: <b>${canUseFeature(tier.tier, 'api_modules') ? 'ENABLED' : 'LOCKED'}</b>\nCustom themes: <b>${canUseFeature(tier.tier, 'custom_themes') ? 'ENABLED' : 'CORE ONLY'}</b>\nAutomation slots: <b>${policy.limits.premiumAutomations < 0 ? 'UNLIMITED' : policy.limits.premiumAutomations}</b>\n\n<i>This test is read-only and does not change entitlement state.</i>`), { parse_mode: 'HTML', reply_markup: proMenu() }); });
bot.command('tools', async (ctx) => ctx.reply(hud(ctx.chat?.id, 'TOOL CATALOG', `Tap any tool to open its detail panel. Page <b>1</b> of <b>${Math.ceil(PRO_TOOLS.length / toolPageSize)}</b>.`), { parse_mode: 'HTML', reply_markup: toolPageKeyboard(0) }));
bot.command('ownercheck', async (ctx) => { const configured = OWNER_IDS.includes(Number(ctx.from?.id)); await ctx.reply(hud(ctx.chat?.id, 'OWNER DIAGNOSTIC', `Your Telegram ID: <code>${ctx.from?.id || 'unknown'}</code>\nOwner match: <b>${configured ? 'CONFIRMED' : 'NOT CONFIGURED'}</b>\n\nSet <code>BOT_OWNER_ID</code> or comma-separated <code>BOT_OWNER_IDS</code> in Render, then redeploy.`), { parse_mode: 'HTML', reply_markup: proMenu() }); });
...
bot.command('theme', async (ctx) => { const chatId = ctx.chat?.id; const name = args(ctx).toLowerCase(); if (!Object.hasOwn(THEMES, name)) return ctx.reply(`Available HUD themes: ${Object.keys(THEMES).join(', ')}`); if (!await requireAdmin(ctx)) return; const tier = tierFor(ctx); if (name !== 'obsidian' && !canUseFeature(tier.tier, 'custom_themes')) return ctx.reply(`${tierPolicy(tier.tier).blockedMessage} Core users can use only the obsidian theme.`); db.setTheme(chatId, name); await ctx.reply(hud(chatId, 'HUD THEME UPDATED', `Active theme: <b>${esc(themeFor(name).name)}</b>`), { parse_mode: 'HTML', reply_markup: proMenu() }); });
bot.command('grantpro', async (ctx) => { if (!OWNER_IDS.includes(Number(ctx.from?.id))) return ctx.reply('Owner access required.'); const chatId = Number(args(ctx)); if (!chatId) return ctx.reply('Usage: /grantpro numeric_chat_id'); db.setPremium(chatId, true); await ctx.reply(`Pro access granted to <code>${chatId}</code>.`, { parse_mode: 'HTML' }); });
bot.command('revokepro', async (ctx) => { if (!OWNER_IDS.includes(Number(ctx.from?.id))) return ctx.reply('Owner access required.'); const chatId = Number(args(ctx)); if (!chatId) return ctx.reply('Usage: /revokepro numeric_chat_id'); db.setPremium(chatId, false); await ctx.reply(`Pro access revoked for <code>${chatId}</code>.`, { parse_mode: 'HTML' }); });
...
bot.callbackQuery(/^tier:(features|test)$/, async (ctx) => { await ctx.answerCallbackQuery(); const chatId = ctx.callbackQuery.message?.chat.id; const tier = tierFor(ctx, chatId); const policy = tierPolicy(tier.tier); if (ctx.match[1] === 'features') return ctx.editMessageText(hud(chatId, 'PRO FEATURES', PRO_FEATURES.map(([name, description]) => `${tier.tier === 'CORE' ? '🔒' : '◆'} <b>${name}</b> — ${description}`).join('\n')), { parse_mode: 'HTML', reply_markup: proMenu() }); return ctx.editMessageText(hud(chatId, 'TIER TEST LAB', `Detected tier: <b>${tier.label}</b>\nAPI access: <b>${canUseFeature(tier.tier, 'api_modules') ? 'ENABLED' : 'LOCKED'}</b>\nCustom themes: <b>${canUseFeature(tier.tier, 'custom_themes') ? 'ENABLED' : 'CORE ONLY'}</b>\nAutomation slots: <b>${policy.limits.premiumAutomations < 0 ? 'UNLIMITED' : policy.limits.premiumAutomations}</b>\n\n<i>Read-only test; no entitlement state is changed.</i>`), { parse_mode: 'HTML', reply_markup: proMenu() }); });
bot.callbackQuery(/^pro:(status|tools|themes|apis)$/, async (ctx) => { await ctx.answerCallbackQuery(); const action = ctx.match[1]; const chatId = ctx.callbackQuery.message?.chat.id; const tier = tierFor(ctx, chatId); if (action === 'status') return ctx.editMessageText(hud(chatId, 'PRO PLATFORM', `Tier: <b>${tier.label}</b>\nTools registered: <b>${PRO_TOOLS.length}</b>\nAPI modules: <b>${API_MODULES.length}</b>\n\n${tier.tier === 'CORE' ? 'Premium modules are locked for this chat.' : 'Premium modules are unlocked.'}`), { parse_mode: 'HTML', reply_markup: proMenu() }); if (action === 'tools') return ctx.editMessageText(hud(chatId, 'TOOL CATALOG', `Tap any tool to open its detail panel. Page <b>1</b> of <b>${Math.ceil(PRO_TOOLS.length / toolPageSize)}</b>.`), { parse_mode: 'HTML', reply_markup: toolPageKeyboard(0) }); if (action === 'themes') return ctx.editMessageText(hud(chatId, 'HUD THEMES', `Available themes: <b>${Object.keys(THEMES).join(' · ')}</b>\n\nUse <code>/theme name</code> as an administrator.`), { parse_mode: 'HTML', reply_markup: proMenu() }); if (tier.tier === 'CORE') return ctx.answerCallbackQuery({ text: 'Pro access required.', show_alert: true }); return ctx.editMessageText(hud(chatId, 'API MODULES', `${renderToolCatalog('Integration')}\n\n<i>${esc(safeApiPolicy())}</i>`), { parse_mode: 'HTML', reply_markup: proMenu() }); });
bot.callbackQuery(/^tools:page:(\d+)$/, async (ctx) => { await ctx.answerCallbackQuery(); const chatId = ctx.callbackQuery.message?.chat.id; const page = Number(ctx.match[1]); return ctx.editMessageText(hud(chatId, 'TOOL CATALOG', `Tap any tool to open its detail panel. Page <b>${page + 1}</b> of <b>${Math.ceil(PRO_TOOLS.length / toolPageSize)}</b>.`), { parse_mode: 'HTML', reply_markup: toolPageKeyboard(page) }); });
bot.callbackQuery(/^tool:view:(\d+)$/, async (ctx) => { const index = Number(ctx.match[1]); const tool = PRO_TOOLS[index]; if (!tool) return ctx.answerCallbackQuery({ text: 'Tool not found.', show_alert: true }); const chatId = ctx.callbackQuery.message?.chat.id; const tier = tierFor(ctx, chatId); if (['Owner', 'Integration'].includes(tool.category) && tier.tier === 'CORE') return ctx.answerCallbackQuery({ text: 'Pro access required.', show_alert: true }); await ctx.answerCallbackQuery(); return ctx.editMessageText(hud(chatId, tool.name.toUpperCase(), `Module ID: <code>${tool.id}</code>\nCategory: <b>${tool.category}</b>\n\n${esc(tool.description)}\n\nStatus: <b>REGISTERED</b>\nTier: <b>${tier.label}</b>\n\n<i>Use the module command or integration configuration documented in the repository to activate this tool.</i>`), { parse_mode: 'HTML', reply_markup: toolPageKeyboard(Math.floor(index / toolPageSize)) }); });
...
bot.callbackQuery(/^confirm:(mute|ban|kick):(-?\d+):(\d+)$/, async (ctx) => { const chatId = Number(ctx.match[2]); const userId = Number(ctx.match[3]); if (!(await isAdminFor(ctx, chatId))) return ctx.answerCallbackQuery({ text: 'Admin access required.', show_alert: true }); await ctx.answerCallbackQuery('Action confirmed');
  const progress = ['⏳ <b>Applying moderation action</b> ·', '⏳ <b>Applying moderation action</b> ··', '⏳ <b>Applying moderation action</b> ···'];
  for (const frame of progress) { await ctx.editMessageText(frame, { parse_mode: 'HTML' }); await new Promise((resolve) => setTimeout(resolve, 180)); }
  await applyModeration(ctx, ctx.match[1], chatId, userId, `user ${userId}`); });
...
const proCommands = [['pro', 'Open the pro platform HUD'], ['profeatures', 'Show Pro features'], ['tiertest', 'Test the current tier'], ['tools', 'Browse the 100+ tool registry'], ['apis', 'Browse opt-in API modules'], ['plugins', 'Inspect the plugin registry'], ['theme', 'Select a HUD theme'], ['ownercheck', 'Diagnose owner access'], ['grantpro', 'Owner: grant Pro access'], ['revokepro', 'Owner: revoke Pro access']];
